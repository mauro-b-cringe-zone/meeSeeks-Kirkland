                ...
                while True:
                    try:
                        ...
                        if reaction.emoji == self.emojis[1]:
                            cogsR = int(cogsR) - 1
                            await msg.edit(embed=await self.ayuda_reaccionada(ctx, str(cogsR), cogs, paginasTotales, discord.Embed(title=f"-=-=-=-=-= Ayuda {cogsR if int(cogsR) < 1 else '9'} -=-=-=-=-=", color=int(env["COLOR"])).set_thumbnail(url="https://raw.githubusercontent.com/maubg-debug/maubot/main/docs/maubot-help-icon.png")))                        
                        ...
                        if reaction.emoji == self.emojis[3]:
                            cogsR = int(cogsR) + 1
                            await msg.edit(embed=await self.ayuda_reaccionada(ctx, str(cogsR), cogs, paginasTotales, discord.Embed(title=f"-=-=-=-=-= Ayuda {cogsR if int(cogsR) > paginasTotales else '9'} -=-=-=-=-=", color=int(env["COLOR"])).set_thumbnail(url="https://raw.githubusercontent.com/maubg-debug/maubot/main/docs/maubot-help-icon.png")))
