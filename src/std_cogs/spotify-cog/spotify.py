...
class Spotify(commands.Cog):
    ...
    def custom_panel(self, title="Title text", subtitle="Subtitle text", description="Description text here", icon="https://cdn.discordapp.com/embed/avatars/0.png", spt=None):
        ...
        if SPOTIFY:
            ...
            DRAW.rectangle([(MARGIN_LEFT, MARGIN_TOP + 95), (MARGIN_RIGHT, MARGIN_TOP + 95)], fill=tuple(map(lambda x: x - 25, BACKGROUND_COLOR)))
            DRAW.rectangle([(MARGIN_LEFT, MARGIN_TOP + 95), ((SEEK / 100 * (MARGIN_RIGHT - MARGIN_LEFT)) + MARGIN_LEFT, MARGIN_TOP + 95)], fill="#2ecc71")
            DRAW.text((MARGIN_LEFT, MARGIN_TOP + 100), STR_CURRENT, font=SUBTITLE_FONT, fill="#2ecc71")
            DRAW.text((MARGIN_RIGHT - DURATION_LEFT_SIZE, MARGIN_TOP + 100), STR_END, font=SUBTITLE_FONT, fill="#2ecc71")
