...
class Mates(commands.Cog):
    ...
    @commands.command(description="Sumar dos numeros")
    ...
    @commands.command(description="restar dos numeros")
    @commands.cooldown(1, 5, commands.BucketType.user)
    async def restar(self, ctx, numOne: int, numTwo: int):
        embed = discord.Embed(title=f"{numOne} - {numTwo} = {numOne - numTwo}", colour=color)
        await ctx.send(embed=embed)

    @commands.command(description="Raiz cuadrada de un numero")
    @commands.cooldown(1, 5, commands.BucketType.user)
